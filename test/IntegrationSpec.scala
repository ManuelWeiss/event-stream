...
case class Msg(stream: String, text: String, user: String, date: String)
...
class IntegrationSpec extends Specification {
  ...
  "Application" should {
    ...
    "work from within a browser" in {
      ...
      "deliver JSON over SSE, filtered by chatStream" in {
        running(TestServer(3333)) {
          ...
          val chatStream = "stream1"

          /** Test data:  */
          val msgs = Seq(
            Json.toJson(Msg("stream1", "message1", "user", "date")),
            Json.toJson(Msg("stream2", "message2", "user", "date")),
            Json.toJson(Msg("stream1", "message3", "user", "date")),
            Json.toJson(Msg("stream3", "message4", "user", "date")),
            Json.toJson(Msg("stream1", "message5", "user", "date")),
            Json.toJson(Msg("stream1", "message6", "user", "date"))
          )

          /** workaround for problems matching within await{}.map{} */
          var resultSeq = Seq[JsValue]()

          /** start stream consuming connection for chat stream feed */
          val client = WS.url("http://localhost:3333/chatFeed/" + chatStream).get(_ => take(n))
          ...
          /** result must be equal to n elements taken from msgs, filtered by chatStream */
          resultSeq.length must beEqualTo(n)
          resultSeq must beEqualTo(msgs.filter(json => (json \ "stream").as[String] == chatStream).take(n))
        }
      }
    }
  }
